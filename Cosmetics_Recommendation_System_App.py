...
# Import the Dataset
skincare = pd.read_csv("Myntra final.csv", encoding="latin-1", low_memory=False)
...
st.sidebar.image(makeup, use_container_width=True)
...
if selected == "Get Recommendation ✨":
    ...
    st.markdown("<h2 style='text-align: center; color: #e84393;'>Discover Your Beauty Favorites! 💫</h2>", unsafe_allow_html=True)
    st.write("---")

    # Create two columns for dropdown layout
    first, last = st.columns(2)

    # Choose product category
    category = first.selectbox(
        label="Product Category 📦",
        options=skincare["product_type"].unique()
    )
    category_pt = skincare[skincare["product_type"] == category]

    # ✅ Skin type dropdown (fixed version)
    skin_type = last.selectbox(
        label="Your Skin Type 🧴",
        options=["All"] + sorted(skincare["skintype"].unique())
    )

    if skin_type != "All":
        category_st_pt = category_pt[category_pt["skintype"] == skin_type]
    else:
        category_st_pt = category_pt
...
if selected == "Skin Care Guide 📚":
    ...
    st.image(image, caption="Skin Care Guide", use_container_width=True)
